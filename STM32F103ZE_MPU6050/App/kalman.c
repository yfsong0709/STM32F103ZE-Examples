...
float Angle_X_Kalman; //X最终倾斜角度
float Angle_Y_Kalman; //Y最终倾斜角度
...
float Q_gyro  = 0.003;
float R_angle = 0.5;
float dt      = 0.02;//dt为kalman滤波器采样时间;
char  C_0     = 1;
...
{
	Angle_Y_Kalman += (Gyro - Q_bias) * dt; //先验估计
	...
	Angle_err = Accel - Angle_Y_Kalman;	//zk-先验估计
	...
	Angle_Y_Kalman += K_0 * Angle_err;	 //后验估计
...
{
	Angle_X_Kalman += (Gyro - Q_bias) * dt; //先验估计
	...
	Angle_err = Accel - Angle_X_Kalman;	//zk-先验估计
	...
	Angle_X_Kalman	+= K_0 * Angle_err;	 //后验估计
	Q_bias	+= K_1 * Angle_err;	 //后验估计
